...
// --- CONSTANTS ---
const uint8_t Prime[P384_LEN] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF };
const uint8_t Order[P384_LEN] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC7, 0x63, 0x4D, 0x81, 0xF4, 0x37, 0x2D, 0xDF, 0x58, 0x1A, 0x0D, 0xB2, 0x48, 0xB0, 0xA7, 0x7A, 0xEC, 0xEC, 0x19, 0x6A, 0xCC, 0xC5, 0x29, 0x73 };
const uint8_t R2Order[P384_LEN] = { 0xD4, 0x0D, 0x49, 0x17, 0x4A, 0xAB, 0x1C, 0xC5, 0xBF, 0x03, 0x06, 0x06, 0xDE, 0x60, 0x9F, 0x43, 0xCC, 0x96, 0x01, 0xF9, 0xF4, 0xA0, 0xE7, 0x92, 0x0C, 0x42, 0xC9, 0x8A, 0xA7, 0x2D, 0x2D, 0x8E, 0x43, 0xBC, 0xF7, 0x15, 0x39, 0x90, 0x00, 0xED, 0x42, 0xA6, 0xFC, 0x1A, 0x99, 0x56, 0x28, 0x11 };
const uint8_t R2Prime[P384_LEN] = { 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFE, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x01 };
...
void verify_ecdsa_final_integrated(
    BIGNUM* gx_mont_stored, BIGNUM* gy_mont_stored,
    BIGNUM* qx_mont_stored, BIGNUM* qy_mont_stored,
    BIGNUM* s, BIGNUM* r, BIGNUM* e_hash,
    BN_CTX* ctx, EC_GROUP* group,
    BN_MONT_CTX* mont_ctx_scalar,
    BN_MONT_CTX* mont_ctx_field,
    BIGNUM* r2_order_bn, BIGNUM* order_bn
) {
    printf("\n========================================================\n");
    printf("VERIFICATION START (Using Stored Montgomery Points)\n");
    printf("========================================================\n");
    ...
    print_bn("Stored Gx (Mont)", gx_mont_stored);
    ...
    print_bn("Stored Qx (Mont)", qx_mont_stored);
    ...
    EC_POINT_mul(group, P1, NULL, Point_G, u1, ctx);
    EC_POINT_mul(group, P2, NULL, Point_Q, u2, ctx);

    BIGNUM *tx = BN_CTX_get(ctx), *ty = BN_CTX_get(ctx);
    printf("\n[Intermediate Points]\n");
    EC_POINT_get_affine_coordinates_GFp(group, P1, tx, ty, ctx);
    print_bn("P1.x (u1*G)", tx);
    EC_POINT_get_affine_coordinates_GFp(group, P2, tx, ty, ctx);
    print_bn("P2.x (u2*Q)", tx);
    ...
    print_bn("v (x1 mod n)", x_mod_n);
    print_bn("r (signature)", r);

    if (BN_cmp(x_mod_n, r) == 0) printf("\n>>> RESULT: MATCH <<<\n");
    else printf("\n>>> RESULT: MISMATCH <<<\n");

    EC_POINT_free(Point_G); EC_POINT_free(Point_Q);
    EC_POINT_free(P1); EC_POINT_free(P2); EC_POINT_free(R_prime);
}
...
int main() {
    ...
    BN_MONT_CTX *mont_ctx_scalar = BN_MONT_CTX_new(); BN_MONT_CTX_set(mont_ctx_scalar, order, ctx);
    BN_MONT_CTX *mont_ctx_field = BN_MONT_CTX_new(); BN_MONT_CTX_set(mont_ctx_field, prime, ctx);
    ...
    // --- MEMORY SIMULATION (Convert Standard -> Mont) ---
    printf("--- [INIT] PREPARING MEMORY (Converting to Mont) ---\n");
    BIGNUM* gx_std = BN_CTX_get(ctx); BN_bin2bn(GenX, P384_LEN, gx_std);
    BIGNUM* gy_std = BN_CTX_get(ctx); BN_bin2bn(GenY, P384_LEN, gy_std);
    BIGNUM* qx_std = BN_CTX_get(ctx); BN_bin2bn(Key, P384_LEN, qx_std);
    BIGNUM* qy_std = BN_CTX_get(ctx); BN_bin2bn(Key + P384_LEN, P384_LEN, qy_std);

    print_bn("GenX Raw", gx_std);

    // Convert: X_mont = MontMul(X, R2_prime)
    BIGNUM* gx_mem = BN_CTX_get(ctx); BN_mod_mul_montgomery(gx_mem, gx_std, r2_prime, mont_ctx_field, ctx);
    BIGNUM* gy_mem = BN_CTX_get(ctx); BN_mod_mul_montgomery(gy_mem, gy_std, r2_prime, mont_ctx_field, ctx);
    BIGNUM* qx_mem = BN_CTX_get(ctx); BN_mod_mul_montgomery(qx_mem, qx_std, r2_prime, mont_ctx_field, ctx);
    BIGNUM* qy_mem = BN_CTX_get(ctx); BN_mod_mul_montgomery(qy_mem, qy_std, r2_prime, mont_ctx_field, ctx);
    
    print_bn("GenX Stored (Mont)", gx_mem);
