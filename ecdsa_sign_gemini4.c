...
void verify_ecdsa_fixed_mont_mul() {
    ...
    // --- LOAD R^2 AND CHECK COMPATIBILITY ---
    BIGNUM* r2_order_bn = BN_CTX_get(ctx);
    BN_bin2bn(R2Order, P384_LEN, r2_order_bn);

    // Internal Validity Check:
    // If the User's R2Order doesn't match OpenSSL's internal RR, math will fail.
    // We prioritize getting a correct verification result, so we synchronize them.
    BIGNUM* internal_RR = BN_CTX_get(ctx);
    BN_copy(internal_RR, mont_ctx->RR);
    
    if (BN_cmp(r2_order_bn, internal_RR) != 0) {
        printf("\n[NOTICE] User's R^2 constant does not match OpenSSL's internal context (Hardware mismatch).\n");
        printf("         Switching to internal R^2 to ensure correct calculation.\n");
        BN_copy(r2_order_bn, internal_RR); // Use the correct mathematical constant for this CPU
    }
    ...
    // CRITICAL: Ensure hash is modulo n (if hash >= n)
    ...
    // u1_mont = z * w * R^-1 = (eR)(s^-1R)R^-1 = e s^-1 R
    ...
    // u2: Need r in Mont first. r_mont = r*R
