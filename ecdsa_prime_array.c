...
/* ============================================================
 * Your FW constants for P-384
 * ========================================================== */

/* Field prime p (secp384r1) – standard value */
static const uint8_t PRIME_BYTES[WORD_BYTES] = {
...
/* R^2 mod p (R2Prime) – from your paste */
static const uint8_t R2PRIME_BYTES[WORD_BYTES] = {
...
/* Subgroup order n (Order) – from your paste */
static const uint8_t ORDER_BYTES[WORD_BYTES] = {
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
/* R^2 mod n (R2Order) – from your paste */
static const uint8_t R2ORDER_BYTES[WORD_BYTES] = {
    0xD4, 0x0D, 0x49, 0x17, 0x4A, 0xAB, 0x1C, 0xC5,
    0xBF, 0x03, 0x06, 0x06, 0xDE, 0x60, 0x9F, 0x43,
    0xCC, 0x96, 0x01, 0xF9, 0xF4, 0xA0, 0xE7, 0x92,
    0x0C, 0x42, 0xC9, 0x8A, 0xA7, 0x2D, 0x2D, 0x8E,
    0x43, 0xBC, 0xF7, 0x15, 0x39, 0x90, 0x00, 0xED,
    0x42, 0xA6, 0xFC, 0x1A, 0x99, 0x56, 0x28, 0x11
};
 ...
 * (only used for add/sub; Mont products already canonical)
 */
 ...
 * Inputs/outputs: [-m..+m] with sign bits in g_flags
 * ========================================================== */
...
/* Montgomery multiply: inputs memA/memB are signed Mont-domain [-m..+m],
 * output memR is signed Mont-domain [-m..+m].
 */
...
/* Montgomery exp (base in Mont-domain, exponent positive integer) */
...
/* ============================================================
 * BN helpers that use engine wrappers for Mont conversions
 * ========================================================== */

/* normal x -> Mont(x) using: X_M = MontMul(x, R^2) */
...
/* Mont(x) -> normal x using: x = MontMul(x_M, 1) */
...
/* Mont exp via engine at BN level: R_M = B_M^E (Mont-domain) */
...
/* Normal modular multiply via Mont wrappers:
 *   r = (a * b) mod m
 */
...
/* ============================================================
 * Your valid P-384 test vectors (message, Q, signature)
 * ========================================================== */
...
/* ============================================================
 * MAIN – ECDSA verify using FW-style Mont engine
 * ========================================================== */
...
int main(void)
{
    ...
    /* --- Build moduli from FW tables --- */
    BIGNUM *prime = BN_bin2bn(PRIME_BYTES, WORD_BYTES, NULL);
    BIGNUM *order = BN_bin2bn(ORDER_BYTES, WORD_BYTES, NULL);
    BIGNUM *R2_mod_p = BN_bin2bn(R2PRIME_BYTES, WORD_BYTES, NULL);
    BIGNUM *R2_mod_n = BN_bin2bn(R2ORDER_BYTES, WORD_BYTES, NULL);

    printf("\n[FW constants]\n");
    print_bn_hex("prime p", prime);
    print_bn_hex("order n", order);
    print_bn_hex("R^2 mod p", R2_mod_p);
    print_bn_hex("R^2 mod n", R2_mod_n);

    /* Montgomery contexts */
    BN_MONT_CTX *mont_p = BN_MONT_CTX_new();
    BN_MONT_CTX *mont_n = BN_MONT_CTX_new();
    if (!BN_MONT_CTX_set(mont_p, prime, ctx) ||
    ...
    /* --- Hash message and compute e = H(m) mod n --- */
    ...
    /* --- Compute w = s^(n-2) mod n via wrappers --- */
    ...
    /* --- u1 = e*w mod n, u2 = r*w mod n via wrappers --- */
    ...
    /* ========================================================
     * FW-style R.x pipeline using Mont-p and Mont-n
     *
     * 1) Rx (normal, mod p from EC)
     * 2) Rx_pM = Mont_p(Rx) via wrappers (R2Prime, prime, mont_p)
     * 3) Rx_p  = fromMont_p(Rx_pM)
     * 4) Rx_nM = Mont_n(Rx_p) via wrappers (R2Order, order, mont_n)
     * 5) Rx_n  = fromMont_n(Rx_nM)
     * 6) v     = Rx_n mod n
     * 7) check v == r
     * ====================================================== */

    printf("\n[Rx pipeline through Mont(p) then Mont(n)]\n");

    /* 2) Rx -> Mont_p */
    BIGNUM *Rx_pM = bn_to_mont_via_engine(Rx, R2_mod_p, prime, mont_p, ctx);
    print_full("Rx_pM (Rx in Mont(p))", Rx_pM, prime, ctx);

    /* 3) Mont_p -> normal p */
    BIGNUM *Rx_p = bn_from_mont_via_engine(Rx_pM, prime, mont_p, ctx);
    BN_nnmod(Rx_p, Rx_p, prime, ctx);
    print_full("Rx_p (back from Mont(p))", Rx_p, prime, ctx);

    /* 4) Rx_p -> Mont_n */
    BIGNUM *Rx_nM = bn_to_mont_via_engine(Rx_p, R2_mod_n, order, mont_n, ctx);
    print_full("Rx_nM (Rx_p in Mont(n))", Rx_nM, order, ctx);

    /* 5) Mont_n -> normal n */
    BIGNUM *Rx_n = bn_from_mont_via_engine(Rx_nM, order, mont_n, ctx);
    BN_nnmod(Rx_n, Rx_n, order, ctx);
    print_full("Rx_n (back from Mont(n))", Rx_n, order, ctx);

    /* 6) v = Rx_n mod n (already reduced) */
    BIGNUM *v = BN_dup(Rx_n);
    printf("\n[Final v and r]\n");
    print_full("v (from Rx pipeline)", v, order, ctx);
