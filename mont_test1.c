...
static void test_add_sub(const MONT_CTX_WR *mc, BN_CTX *ctx, int m)
{
    ...
    for (int i = 0; i < ncases; ++i) {
        ...
        printf("[ADD] a=%d, b=%d -> r=%d, expected=%d  %s\n",
               a, b, r_int, exp, (r_int == exp) ? "OK" : "FAIL");

        if (r_int == exp) pass_add++;
        ...
        printf("[SUB] a=%d, b=%d -> r=%d, expected=%d  %s\n",
               a, b, r_int, exp, (r_int == exp) ? "OK" : "FAIL");

        if (r_int == exp) pass_sub++;
