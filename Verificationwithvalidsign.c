...
/* ================================================
 * MESSAGE (your exact 128-byte original message)
 * ================================================ */
...
/* ================================================
 * PUBLIC KEY Q (your correct 96-byte Qx||Qy)
 * ================================================ */
...
/* ================================================
 * SIGNATURE (r||s) — your valid signature
 * ================================================ */
...
{
    printf("\n===========================================\n");
    printf("   GOLDEN REFERENCE ECDSA P-384 VERIFIER\n");
    printf("===========================================\n");
    ...
    /* ============ Manual inverse-based verify ============ */
    printf("\n--- Manual inverse-based verify ---\n");

    BIGNUM *n = BN_new();
    EC_GROUP_get_order(group, n, ctx);

    BIGNUM *w = BN_mod_inverse(NULL, s, n, ctx);
    print_bn("w = s^{-1} mod n", w);

    BIGNUM *u1 = BN_new(), *u2 = BN_new();
    BN_mod_mul(u1, e, w, n, ctx);
    BN_mod_mul(u2, r, w, n, ctx);

    print_bn("u1 = e*w mod n", u1);
    print_bn("u2 = r*w mod n", u2);
    ...
    BIGNUM *w2 = BN_new();
    BN_mod_exp(w2, s, exp, n, ctx);
    print_bn("w2 = s^(n-2)", w2);

    /* Check equality */
    printf("Compare inverse vs pow: %s\n",
           BN_cmp(w, w2) == 0 ? "MATCH" : "MISMATCH");

    BIGNUM *u1b = BN_new(), *u2b = BN_new();
    BN_mod_mul(u1b, e, w2, n, ctx);
    BN_mod_mul(u2b, r, w2, n, ctx);

    print_bn("u1b = e*w2 mod n", u1b);
    print_bn("u2b = r*w2 mod n", u2b);

    EC_POINT *R2 = EC_POINT_new(group);
    EC_POINT_mul(group, R2, u1b, Q, u2b, ctx);

    BIGNUM *Rx2 = BN_new(), *Ry2 = BN_new();
    EC_POINT_get_affine_coordinates(group, R2, Rx2, Ry2, ctx);

    print_bn("Rx2", Rx2);

    BIGNUM *v2 = BN_new();
    BN_nnmod(v2, Rx2, n, ctx);
    print_bn("v2 = Rx2 mod n", v2);

    printf("FW-style verify: %s\n", BN_cmp(v2, r) == 0 ? "VALID" : "INVALID");
