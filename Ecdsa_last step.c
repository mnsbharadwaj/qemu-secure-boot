#include <openssl/bn.h>
#include <openssl/ec.h>
#include <openssl/obj_mac.h>
#include <openssl/err.h>
#include <openssl/sha.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#define P384_LEN 48

// --- 1. USER PROVIDED ARRAYS ---

// PRIME P
const uint8_t Prime[P384_LEN] = { 
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE, 
    0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF
};

// ORDER N
const uint8_t Order[P384_LEN] = { 
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xC7, 0x63, 0x4D, 0x81, 0xF4, 0x37, 0x2D, 0xDF, 
    0x58, 0x1A, 0x0D, 0xB2, 0x48, 0xB0, 0xA7, 0x7A, 0xEC, 0xEC, 0x19, 0x6A, 0xCC, 0xC5, 0x29, 0x73
};

// R^2 MOD ORDER
const uint8_t R2Order[P384_LEN] = { 
    0xD4, 0x0D, 0x49, 0x17, 0x4A, 0xAB, 0x1C, 0xC5, 0xBF, 0x03, 0x06, 0x06, 0xDE, 0x60, 0x9F, 0x43, 
    0xCC, 0x96, 0x01, 0xF9, 0xF4, 0xA0, 0xE7, 0x92, 0x0C, 0x42, 0xC9, 0x8A, 0xA7, 0x2D, 0x2D, 0x8E, 
    0x43, 0xBC, 0xF7, 0x15, 0x39, 0x90, 0x00, 0xED, 0x42, 0xA6, 0xFC, 0x1A, 0x99, 0x56, 0x28, 0x11
};

// GEN X, GEN Y (NORMAL DOMAIN)
const uint8_t GenX[P384_LEN] = { 0xAA, 0x87, 0xCA, 0x22, 0xBE, 0x8B, 0x05, 0x37, 0x8E, 0xB1, 0xC7, 0x1E, 0xF3, 0x20, 0xAD, 0x74, 0x6E, 0x1D, 0x3B, 0x62, 0x8B, 0xA7, 0x9B, 0x98, 0x59, 0xF7, 0x41, 0xE0, 0x82, 0x54, 0x2A, 0x38, 0x55, 0x02, 0xF2, 0x5D, 0xBF, 0x55, 0x29, 0x6C, 0x3A, 0x54, 0x5E, 0x38, 0x72, 0x76, 0x0A, 0xB7 };
const uint8_t GenY[P384_LEN] = { 0x36, 0x17, 0xDE, 0x4A, 0x96, 0x26, 0x2C, 0x6F, 0x5D, 0x9E, 0x98, 0xBF, 0x92, 0x92, 0xDC, 0x29, 0xF8, 0xF4, 0x1D, 0xBD, 0x28, 0x9A, 0x14, 0x7C, 0xE9, 0xDA, 0x31, 0x13, 0xB5, 0xF0, 0xB8, 0xC0, 0x0A, 0x60, 0xB1, 0xCE, 0x1D, 0x7E, 0x81, 0x9D, 0x7A, 0x43, 0x1D, 0x7C, 0x90, 0xEA, 0x0E, 0x5F };

// TEST VECTORS
const uint8_t aMsg[] = {
    
};

const uint8_t Key[] = {
    0x5E, 0xB8, 0x69, 0x6E, 0x47, 0x9F, 0xE9, 0x57, 0xF1, 0xF2, 0xCB, 0xCF, 0xB1, 0x09, 0xA4, 0xD2,
    0xEA, 0x0A, 0x58, 0xCE, 0xDB, 0xEB, 0x70, 0xA0, 0x59, 0x7E, 0x5C, 0x21, 0x09, 0x11, 0x01, 0xDD,
    0x96, 0x95, 0xDB, 0x07, 0x23, 0x7F, 0xDF, 0xC7, 0xC5, 0xC7, 0x2C, 0x55, 0x7F, 0xB5, 0xB8, 0x9B,
    0x5F, 0xC8, 0x0C, 0xF1, 0x22, 0xA6, 0x31, 0x5A, 0x9F, 0x80, 0x97, 0xBC, 0xA3, 0xBE, 0xCD, 0xF2,
    0x72, 0xCF, 0x99, 0xFF, 0x20, 0x41, 0x94, 0x37, 0x38, 0x14, 0xAA, 0x45, 0xAD, 0xE5, 0x75, 0x45,
    0x95, 0xDA, 0x0B, 0xEE, 0x09, 0x85, 0x62, 0x5C, 0xF3, 0x78, 0x61, 0x70, 0x24, 0x00, 0x44, 0x34
};

const uint8_t Signature[] = {
    0x5B, 0xBD, 0x29, 0x46, 0xC5, 0x8E, 0xBF, 0x5C, 0x7D, 0xFE, 0xBD, 0x5C, 0xBE, 0x5A, 0x2D, 0xC0,
    0xF4, 0xE7, 0xA2, 0xA3, 0xB8, 0xD2, 0x63, 0x53, 0xF3, 0xFC, 0x54, 0x58, 0x9D, 0x18, 0x5F, 0xDD,
    0x75, 0xC3, 0x47, 0x21, 0x0D, 0x9B, 0xB2, 0x81, 0x23, 0x41, 0xD3, 0x8E, 0x14, 0xA2, 0x0F, 0x2D,
    0x90, 0xAD, 0xF5, 0x21, 0x2F, 0x03, 0x17, 0xB2, 0x61, 0x39, 0x35, 0xAC, 0x76, 0x5F, 0x90, 0xF3,
    0x72, 0x56, 0xB6, 0xDC, 0x0B, 0x04, 0x1C, 0x33, 0xE8, 0x65, 0xDE, 0x34, 0x44, 0x21, 0x44, 0xD3,
    0x10, 0xE0, 0xCB, 0x6C, 0x10, 0x55, 0x89, 0xD8, 0x60, 0x63, 0xCD, 0xDB, 0xD8, 0x0A, 0x96, 0x15
};

void print_bn(const char* label, const BIGNUM* bn) {
    char* hex = BN_bn2hex(bn);
    printf("  %s: %s\n", label, hex);
    OPENSSL_free(hex);
}

void verify_ecdsa_final() {
    BN_CTX* ctx = BN_CTX_new();
    BN_CTX_start(ctx); 

    EC_GROUP* group = EC_GROUP_new_by_curve_name(NID_secp384r1);
    
    // --- SETUP CONTEXTS ---
    BIGNUM* order = BN_CTX_get(ctx); 
    BN_bin2bn(Order, P384_LEN, order);
    BN_MONT_CTX *mont_ctx_scalar = BN_MONT_CTX_new();
    BN_MONT_CTX_set(mont_ctx_scalar, order, ctx);

    BIGNUM* prime = BN_CTX_get(ctx);
    BN_bin2bn(Prime, P384_LEN, prime);
    BN_MONT_CTX *mont_ctx_field = BN_MONT_CTX_new();
    BN_MONT_CTX_set(mont_ctx_field, prime, ctx);

    // --- SETUP R^2 (Auto-Correct logic) ---
    BIGNUM* r2_order_bn = BN_CTX_get(ctx);
    BN_bin2bn(R2Order, P384_LEN, r2_order_bn);
    
    BIGNUM* local_R2 = BN_CTX_get(ctx);
    BN_zero(local_R2); BN_set_bit(local_R2, BN_num_bits(order)); 
    BN_mod_sqr(local_R2, local_R2, order, ctx);
    if (BN_cmp(local_R2, r2_order_bn) != 0) {
        printf("\n[WARN] R2 Mismatch. Auto-corrected to ensure math works.\n");
        BN_copy(r2_order_bn, local_R2);
    }

    // --- INPUTS ---
    uint8_t digest[SHA384_DIGEST_LENGTH];
    SHA384(aMsg, sizeof(aMsg), digest);
    BIGNUM* e = BN_CTX_get(ctx); BN_bin2bn(digest, SHA384_DIGEST_LENGTH, e);
    BN_nnmod(e, e, order, ctx);

    BIGNUM* r = BN_CTX_get(ctx); BN_bin2bn(Signature, P384_LEN, r);
    BIGNUM* s = BN_CTX_get(ctx); BN_bin2bn(Signature + P384_LEN, P384_LEN, s);

    // =========================================================================
    // STEP 1-5: SCALAR CALCULATION
    // =========================================================================
    // ... [Scalar math remains identical: calculate u1, u2] ...
    
    printf("\n--- STEP 1-5: Calculating u1 and u2 ---\n");
    // 1. Index
    BIGNUM* index = BN_CTX_get(ctx); BIGNUM* two = BN_CTX_get(ctx); BN_set_word(two, 2);
    BN_sub(index, order, two);

    // 2. S_mont
    BIGNUM* S_mont = BN_CTX_get(ctx);
    BN_mod_mul_montgomery(S_mont, s, r2_order_bn, mont_ctx_scalar, ctx);

    // 3. w (Inverse)
    BIGNUM* w = BN_CTX_get(ctx);
    BN_mod_mul_montgomery(w, BN_value_one(), r2_order_bn, mont_ctx_scalar, ctx); 
    int num_bits = BN_num_bits(index);
    for (int i = num_bits - 1; i >= 0; i--) {
        BN_mod_mul_montgomery(w, w, w, mont_ctx_scalar, ctx); 
        if (BN_is_bit_set(index, i)) BN_mod_mul_montgomery(w, w, S_mont, mont_ctx_scalar, ctx);
    }

    // 4. z (Hash Mont)
    BIGNUM* z = BN_CTX_get(ctx);
    BN_mod_mul_montgomery(z, e, r2_order_bn, mont_ctx_scalar, ctx);

    // 5. u1, u2
    BIGNUM* u1_mont = BN_CTX_get(ctx);
    BN_mod_mul_montgomery(u1_mont, z, w, mont_ctx_scalar, ctx);
    BIGNUM* u1 = BN_CTX_get(ctx);
    BN_from_montgomery(u1, u1_mont, mont_ctx_scalar, ctx);
    print_bn("u1", u1);

    BIGNUM* r_mont = BN_CTX_get(ctx);
    BN_mod_mul_montgomery(r_mont, r, r2_order_bn, mont_ctx_scalar, ctx);
    BIGNUM* u2_mont = BN_CTX_get(ctx);
    BN_mod_mul_montgomery(u2_mont, r_mont, w, mont_ctx_scalar, ctx);
    BIGNUM* u2 = BN_CTX_get(ctx);
    BN_from_montgomery(u2, u2_mont, mont_ctx_scalar, ctx);
    print_bn("u2", u2);

    // =========================================================================
    // STEP 6: POINT ARITHMETIC (Explicit Base Points)
    // =========================================================================
    printf("\n--- STEP 6: Point Math (Explicit Points) ---\n");

    // --- 6a. Set up Point_G (From Normal Arrays) ---
    BIGNUM* gx_std = BN_CTX_get(ctx); BN_bin2bn(GenX, P384_LEN, gx_std);
    BIGNUM* gy_std = BN_CTX_get(ctx); BN_bin2bn(GenY, P384_LEN, gy_std);
    
    EC_POINT* Point_G = EC_POINT_new(group);
    EC_POINT_set_affine_coordinates_GFp(group, Point_G, gx_std, gy_std, ctx);
    print_bn("Point G_x (Base)", gx_std);

    // --- 6b. Set up Point_Q (From Key, Mont->Normal) ---
    // NOTE: Assuming Key is in Montgomery. If Key is Normal, remove BN_from_montgomery calls.
    BIGNUM* qx_mont = BN_CTX_get(ctx); BN_bin2bn(Key, P384_LEN, qx_mont);
    BIGNUM* qy_mont = BN_CTX_get(ctx); BN_bin2bn(Key + P384_LEN, P384_LEN, qy_mont);
    
    BIGNUM* qx_std = BN_CTX_get(ctx);
    BN_from_montgomery(qx_std, qx_mont, mont_ctx_field, ctx);
    BIGNUM* qy_std = BN_CTX_get(ctx);
    BN_from_montgomery(qy_std, qy_mont, mont_ctx_field, ctx);

    EC_POINT* Point_Q = EC_POINT_new(group);
    EC_POINT_set_affine_coordinates_GFp(group, Point_Q, qx_std, qy_std, ctx);
    print_bn("Point Q_x (Public Key)", qx_std);

    // --- 6c. Calculate Terms ---
    EC_POINT* Term1 = EC_POINT_new(group);
    EC_POINT* Term2 = EC_POINT_new(group);
    EC_POINT* R_prime = EC_POINT_new(group);

    // Term1 = u1 * Point_G
    // EC_POINT_mul(group, result, NULL, point, scalar, ctx)
    EC_POINT_mul(group, Term1, NULL, Point_G, u1, ctx);
    
    // Term2 = u2 * Point_Q
    EC_POINT_mul(group, Term2, NULL, Point_Q, u2, ctx);

    [attachment_0](attachment)

    // R' = Term1 + Term2
    EC_POINT_add(group, R_prime, Term1, Term2, ctx);

    BIGNUM* x1 = BN_CTX_get(ctx);
    EC_POINT_get_affine_coordinates_GFp(group, R_prime, x1, NULL, ctx);
    print_bn("Result x1", x1);

    // =========================================================================
    // STEP 7: COMPARE
    // =========================================================================
    printf("\n--- STEP 7: Compare ---\n");
    BIGNUM* x_mod_n = BN_CTX_get(ctx);
    BN_mod(x_mod_n, x1, order, ctx);
    
    print_bn("v (calculated)", x_mod_n);
    print_bn("r (signature) ", r);

    if (BN_cmp(x_mod_n, r) == 0) {
        printf("\n>>> MATCH: VALID SIGNATURE <<<\n");
    } else {
        printf("\n>>> MISMATCH <<<\n");
    }

    BN_MONT_CTX_free(mont_ctx_scalar);
    BN_MONT_CTX_free(mont_ctx_field);
    BN_CTX_end(ctx);
    BN_CTX_free(ctx);
    EC_GROUP_free(group);
    EC_POINT_free(Point_G); EC_POINT_free(Point_Q);
    EC_POINT_free(Term1); EC_POINT_free(Term2); EC_POINT_free(R_prime);
}

int main() {
    ERR_load_crypto_strings();
    verify_ecdsa_final();
    ERR_free_strings();
    return 0;
}
