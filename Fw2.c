 ...
 *     sig_bin[96] (r‖s)
 * - Uses:
 *     * s^(n-2) mod n instead of inverse
 *     * Montgomery mod n for all scalar arithmetic
 *     * "Engine" wrappers with sign bits in a global word
 *     * Final Step-7 pipeline:
 *          X in mod p
 *          MontEnc_p -> MontDec_p
 *          MontEnc_n -> MontDec_n
 *          compare with r
...
/*---------------------------------------------------------
 * Global flags word for sign bits
 *   bit0  : sign of operand A
 *   bit1  : sign of operand B
 *   bit31 : sign of result
 *--------------------------------------------------------*/
...
/* symmetric: (-mod/2, +mod/2], only for printing/debug */
...
/*---------------------------------------------------------
 * Magnitude + sign-bit encoding (48-byte big-endian)
 *--------------------------------------------------------*/
...
/*---------------------------------------------------------
 * Montgomery multiply / exponent mod n (core math)
 *--------------------------------------------------------*/
...
/*---------------------------------------------------------
 * "Engine" wrappers mod n using sign bits and 48-byte mem
 *--------------------------------------------------------*/
...
/*---------------------------------------------------------
 * Inputs: same as you gave initially
 *--------------------------------------------------------*/
...
/*---------------------------------------------------------
 * MAIN – ECDSA VERIFY WITH YOUR ENGINE + STEP-7
 *--------------------------------------------------------*/
int main(void)
{
    ...
    /* Order n (scalar field) and field prime p */
    BIGNUM *n = BN_new();
    ...
    /******** STEP 1: Load r and s from sig_bin ********/
    ...
    /******** STEP 4: temp1 = s^(n-2) mod n using engine_exp_n ********/
    ...
    /******** STEP 6: EC two-term scalar multiply:
     *  R = Scalar1 * Q + Scalar0 * G
     *********************************************************/
    ...
    /******** STEP 7: Your final pipeline
     *   X_p = Rx mod p
     *   X_pM      = MontEnc_p(X_p)
     *   X_p_round = MontDec_p(X_pM)
     *   X_nM      = MontEnc_n(X_p_round mod n)
     *   X_final   = MontDec_n(X_nM)
     *   compare X_final with r
     *********************************************************/
    BIGNUM *X_p       = BN_new();
    BIGNUM *X_pM      = BN_new();
    BIGNUM *X_p_round = BN_new();
    BIGNUM *v0        = BN_new();
    BIGNUM *v0M       = BN_new();
    BIGNUM *X_final   = BN_new();

    /* X_p = Rx mod p */
    BN_nnmod(X_p, Rx, (BIGNUM *)p, ctx);
    print_bn_stage_mod("X_p = Rx mod p", X_p, (BIGNUM *)p, ctx);

    /* Mont(p) encode/decode */
    BN_to_montgomery(X_pM, X_p, g_mont_p, ctx);
    print_bn_stage_mod("X_p in Mont(p)", X_pM, (BIGNUM *)p, ctx);

    BN_from_montgomery(X_p_round, X_pM, g_mont_p, ctx);
    print_bn_stage_mod("X_p after Mont(p) round-trip", X_p_round, (BIGNUM *)p, ctx);

    /* Move to order n domain through Mont(n) */
    BN_nnmod(v0, X_p_round, n, ctx);
    print_bn_stage_mod("v0 = X_p_round mod n", v0, n, ctx);

    BN_to_montgomery(v0M, v0, g_mont_n, ctx);
    print_bn_stage_mod("v0 in Mont(n)", v0M, n, ctx);

    BN_from_montgomery(X_final, v0M, g_mont_n, ctx);
    print_bn_stage_mod("X_final after Mont(n) round-trip", X_final, n, ctx);

    /******** STEP 8: Final comparison ********/
    if (BN_cmp(X_final, r) == 0)
        printf("\n✔ SIGNATURE VALID (X_final == r)\n");
    else
        printf("\n✘ SIGNATURE INVALID (X_final != r)\n");
